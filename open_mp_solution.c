...
int main (int argc, char *argv[])
{
    ...
    #pragma omp parallel for private(x, y, loc) collapse(2)
    for (int i=1; i<M+1; i++){
        x = A1 + i * h1;
        for (int j=1; j<N+1; j++){
            y = A2 + j * h2;
    ...
    for (int k = 0; threshold < rr; k++) {
        ...
        // Параллельная область охватывает все три цикла
        #pragma omp parallel
        {

            // Первый цикл: вычисление Aw_matrix и r_matrix
            ...
            // Второй цикл: вычисление rr и Arr с reduction
            #pragma omp for reduction(+:rr) reduction(+:Arr)
            for (int i = 1; i < M; i++) {
                for (int j = 1; j < N; j++) {
            ...
            // Вычисление tau вне цикла, так как это коллективная операция
            #pragma omp single 
            tau = rr / Arr;

            // Третий цикл: обновление w_matrix
            #pragma omp for
            for (int i = 1; i < M; i++) {
                for (int j = 1; j < N; j++) {
        ...
        // Условие остановки
        if (rr < threshold) {
            printf("success! rr=%f, threshold=%f\n", rr, threshold);
        }
    }
